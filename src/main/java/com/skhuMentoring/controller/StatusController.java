...
public class StatusController {
    ...
    public String detailMentor(Model model, Long bno, String mentorId) {
        ...
        model.addAttribute("mentorMentoringCount", myPageService.getMentorCount(mentorId)); // 멘토가 현재까지 진행한 멘토링 횟수
        model.addAttribute("getMentoringCount", myPageService.getMentoringCount(mentorId)); // 해당 멘토의 현재까지 가르킨 멘티 수
    ...
    public String detailMentee(Model model, Long bno, Mentee mentee, String menteeId) {
        model.addAttribute("detailMentee", myPageService.getDetailMentee(bno, menteeId));
        model.addAttribute("menteeMentoringCount", myPageService.getMenteeCount(menteeId)); // 멘티가 현재까지 들은 멘토링 횟수
